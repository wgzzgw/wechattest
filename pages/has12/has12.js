...
Page({
  ...
  onLoad: function () {
    this.setData({
      datasource: app.globalData.HAS12,
      dataanswer: app.globalData.HAS12answer
    })
  },
  //随机排序
  randSort: function () {
  ...
  initNumber: function () {
    var result = this.data.dataanswer[this.data.list[this.data.index % 171]];//答案
    //开始随机生成成4个答案
  ...
  runTime: function (that) {
    if (that.data.minute == 0 && that.data.second == 0) {
      wx.redirectTo({
        url: '../has12result/has12result?rightAmount=' + this.data.rightAmount
