...
Page({
  ...
  onLoad: function () {
    this.setData({
      datasource: app.globalData.HAS121,
      dataanswer: app.globalData.HAS121answer
    })
  },
  //随机排序
  randSort: function () {
  ...
  runTime: function (that) {
    if (that.data.minute == 0 && that.data.second == 0) {
      wx.redirectTo({
        url: '../has121result/has121result?rightAmount=' + this.data.rightAmount
