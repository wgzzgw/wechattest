...
Page({
  ...
  onLoad: function () {
    this.setData({
      datasource: app.globalData.HAS122,
      dataanswer: app.globalData.HAS122answer
    })
  },
  //随机排序
  randSort: function () {
  ...
  runTime: function (that) {
    if (that.data.minute == 0 && that.data.second == 0) {
      wx.redirectTo({
        url: '../has122result/has122result?rightAmount=' + this.data.rightAmount
