...
Page({
  ...
  data: {
    ...
    248,250,251,252,253,254,255,256,257,258,259,260,261,262,263,264,265,266,267,268,269,270,271,272,273,274,275,276,277,278,279,280,281,282,283,284,285,286,287,288,289,290,291,292,293,294,295,296,297,298,299,300,301,302,303,304,305,306,
  ...
  onLoad: function () {
    this.setData({
      datasource: app.globalData.HAS1,
      dataanswer: app.globalData.HAS1answer
    })
  },
  //随机排序
  randSort: function () {
  ...
  initNumber: function () {
    var result = this.data.dataanswer[this.data.list[this.data.index%532]];//答案
    //开始随机生成成4个答案
  ...
  runTime: function (that) {
    if (that.data.minute == 0 && that.data.second == 0) {
      wx.redirectTo({
        url: '../has1result/has1result?rightAmount=' + this.data.rightAmount
